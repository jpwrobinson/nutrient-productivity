...
ggplot(prod_reef, aes(prod_day_ha, biomass_turnover)) + geom_point() + facet_wrap(~nutrient, scales='free') +
  stat_smooth(method='gam')
ggplot(prod_reef, aes(biomass_kg, biomass_turnover)) + geom_point() + facet_wrap(~nutrient, scales='free') +
  stat_smooth(method='gam')
ggplot(prod_reef, aes(biomass_kg, nut_turnover)) + geom_point()


pdf(file = 'fig/explore/wcs_nutrient_prod_reef.pdf', height=7, width=12)
ggplot(prod_reef, aes(biomass_kg, nut_prod_day_ha)) + geom_point() + facet_wrap(~nutrient, scales='free')
...
ggplot(prod_reef, aes(biomass_kg, nut_turnover)) + geom_point() + facet_wrap(~nutrient, scales='free')
ggplot(prod_reef, aes(biomass_kg, prod_day)) + geom_point() + facet_wrap(~nutrient, scales='free')
...
prod_sp<-prod %>% 
  ...
  summarise(
    ...
    biomass_kg = mean(biomass_kg)) %>% 
  ungroup() %>% 
  select(country, site, species, nutrient, trophic_group, nut_prod_day_ha:biomass_kg) %>% 
  complete(site, nutrient, nesting(species, trophic_group, country),
           fill = list(nut_prod_day_ha = 0, nut_biomass_kgha = 0, prod_day =0, biomass_kg = 0)) %>%
  # mutate(nut_turnover = nut_prod_day_ha / nut_biomass_kgha,
...
prod_scale<-prod_sp %>% group_by(nutrient) %>% 
      ...
      summarise(nut_prod_score = sum(nut_prod_day_ha_scale),
                biomass_kg = unique(biomass_kg))

pdf(file = 'fig/explore/wcs_nutrient_prod_species.pdf', height=18, width=20)
ggplot(prod_top_nut, aes(fct_reorder(species, nut_prod_day_ha), nut_prod_day_ha, fill=trophic_lab)) + 
