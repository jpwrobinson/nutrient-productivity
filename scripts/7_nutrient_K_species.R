...
mad<-read.csv("data/wcs/madagascar_potential-prod-test_rfishprod.csv") %>% 
  distinct(species, Kmax)

nut<-left_join(nut, mad, by = 'species') %>% droplevels()

ggplot(nut, aes(Kmax, nut_density)) + geom_point() +
      geom_label(data = nut %>% filter(nut_density>400), aes(label = species)) +
      geom_label(data = nut %>% filter(Kmax>0.4), aes(label = species))
