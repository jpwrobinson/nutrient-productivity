...
grav<-data.frame(grav = seq(min(gravS), max(gravS), length.out=100))
gravM<-matrix(replicate(6,grav$grav),nrow=100)

int<-post %>% filter(varname=='intercept') %>% group_by(fg) %>% 
  summarise(m = mean(value)) %>% ungroup() %>% summarise(mu=sum(m))
...
gs<-rowSums(sweep(gravM, 2, colMeans(g), "*"))
...
for(i in 1:6){
  ...
  pp[,i+1]<-mean(temp$value[temp$varname=='intercept']) + 
    mean(temp$value[temp$varname=='gravity'])*grav$grav
}
...
ggplot(pp, aes(gravity, mu, col=fg)) + geom_line()
